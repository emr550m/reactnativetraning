...
export function ResponsiveArea() {
    ...
    return (<View>
        <FlatList styles={styles.list} numColumns={2}
            data={items}
            renderItem={({ item }) =>
                <View style={styles.listItem}><ListItem key={item.key} title={item.title} image={item.image} description={item.description} /></View>}
        />
    </View>)
}
